...
def _create_mapping_files_local(array, mapping_file, savepath):
    ...
    with full_mapping_path.open("r") as full_mapping:
        ...
        for line in full_mapping:
            ...
            if (
                len(split) < 3
            ):  # <3 rather != because some indentifiers contain a space in the third column
            ...
            if split[1] in identifier_dict:
                ...
                if split[2] in ids:
                    ...
                    if split[2] not in dict_of_mapping_dicts[split[1]]:
                        ...
                        dict_of_mapping_dicts[split[1]][split[2]].append(split[0])
    ...
    for mapping_id, mapping_dict in dict_of_mapping_dicts.items():
        ...
        if not mapping_dict:
            print(
                "No {} identifiers found in dataset.\n".format(
                    identifier_dict[mapping_id]
                )
            )
        else:
            path = Path(savepath) / (
                identifier_dict[mapping_id].replace("/", "-").strip(":")
                + r"2uniprot.txt"
            )
            path.parent.mkdir(parents=True, exist_ok=True)
            print(
                "Created mapping file between UniProt ACs and {} in: {}.\n".format(
                    identifier_dict[mapping_id], path.resolve()
                )
            )
            with path.open("w") as mapping_out:
                for ident, uniac in mapping_dict.items():
                    if len(uniac) > 1:
                        ...
                        mapping_out.write(
                            ident + "\t" + "\t" + uniac[0] + "\t" + "reviewed" + "\n"
                        )


def _create_mapping_files(array, from_id, description, savepath, query_chunk_size=1000):
    ...
    if ids.empty:
        ...
        print("{}: {} identifiers found in dataset.".format(description, ids.size))
        ...
        progress_counter = 0
        for i in [
            ids[j : j + query_chunk_size] for j in range(0, len(ids), query_chunk_size)
        ]:
...
def _extract_identifiers(entries):
    ...
    identifier_list = list(pd.Series(entries).str.split(":").str.get(0).unique())
...
def map2uniprot(
    ...
    if not skip_creation:
        ...
        if full_mapping_file:
            ...
            _create_mapping_files_local(
                merged_columns, full_mapping_file, Path(filepath)
            )
    ...
    for db_tag, id_abbreviation in identifier_dict.items():
        ...
        if not skip_creation:
            ...
            if not full_mapping_file:
                ...
                if not path.is_file():
                    _create_mapping_files(merged_columns, id_abbreviation, db_tag, path)
                else:
                    print(
        ...
        if Path(path).is_file():
            ...
            for col in columns:
                ...
                success_series.append(
                    interaction_dataframe.loc[
                        mapping_selection
                        & ~interaction_dataframe[col].str.startswith(db_tag),
                        col,
                    ]
                )
            ...
            print(
                "{} {} out of {} identifiers were succesfully remapped to UniProt accession numbers.\n".format(
                    db_tag, success_count, total_count
                )
            )

    # report how many identifiers had multiple possible remappings
    mult_map_counts = (
        pd.Series(pd.unique(interaction_dataframe[columns].values.ravel("K")))
        .str.contains("MULT_MAP")
        .sum()
    )
    ...
    print(
        '{} identifiers were remapped to multiple UniProt accession numbers (denoted by the prefix "MULT_MAP" in the dataset) for a total of {} interactions.\n'.format(
            mult_map_counts, mult_map_interaction_count
        )
    )
    """
...
def remove_mult(interaction_dataframe, columns=None):
    ...
    selection = ~interaction_dataframe[columns[0]].str.contains(
        "MULT_MAP"
    ) & ~interaction_dataframe[columns[1]].str.contains("MULT_MAP")
...
def check_unique_identifier(interaction_dataframe, columns=None):
    ...
    for i in columns:
        ...
        if any(non_unique_mask):
            interaction_dataframe.loc[non_unique_mask, i] = interaction_dataframe.loc[
                non_unique_mask, i
            ].apply(lambda_helper)
            print(
