...
def remap_GO_depth(interaction_dataframe, depth, go_dict, exception_list=None, GO_columns=None):
    ...
    exception_list : list
        A list containing GO term IDs that should be ignored (i.e. their tracker child should be retained instead).
...
def map_node_to_depth(node, depth, go_dict, found_nodes, exception_list=None, tracker=None):
    ...
    Note: an exception list can be provided. Whenever a term is mapped to an ID in this list, the ID is not saved,
          but its previous caller (tracker) is collected instead.
    ...
    exception_list : list
        ...
        (Default: None, which sets it to an empty list.)
    tracker : str
        Tracks the previous node that called the current iteration of the recursion.
...
if __name__ == '__main__':
    ...
    go_dict = obo_tools.importOBO(r'../go_data/go.obo')
    ...
    remap_GO_depth(df_herpes, depth, go_dict, list(exclusion))
